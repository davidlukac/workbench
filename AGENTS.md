...
AI Agent Workbench — a TypeScript CLI that bridges Jira tickets to AI sub-agent execution via spec expansion, task decomposition, and persona-matched agent dispatch. The coordination layer is a stdio MCP server exposing a workspace-scoped task ledger.
...
**`workbench-emulator`** — Run the full workbench pipeline (story → spec → plan → dispatch → review → signoff) without any MCP infrastructure. Use `/workbench-emulator <story-file-or-id>` to emulate a complete session using only files under `.workbench/emulator/<story-id>/`. Useful for developing and testing the workflow before Phase 2 infrastructure is built.
...
Use `/skill-creator` to create/evaluate/improve skills. See `docs/WhatAreSkills.md` for the Agent Skills open standard.
