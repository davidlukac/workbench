...
You are running the complete AI Agent Workbench pipeline in file-based emulation mode. You play every role sequentially: Workflow orchestrator, Spec Agent, Planner Agent, Story Manager, Implementation Sub-Agent, and Reviewer. No MCP server or CLI is needed — all state lives in files under `.workbench/emulator/`.
...
All emulator state lives under `.workbench/emulator/<story-id>/`:

```
.workbench/emulator/<story-id>/
├── story.md                   # Normalized Story entity
├── spec.md                    # Generated Spec
├── tasks/
│   ├── task-<id>-001.md       # One file per planned task
│   └── task-<id>-NNN.md
└── summary.md                 # Session summary (written last)
```

Create the directory before writing any file.
...
1. **Attempt MCP fetch (preferred):** Call the `fetch_story` workbench MCP tool with `source_ref: <bare path or story ID>`. On success, extract the normalized Story entity fields (`id`, `title`, `description`, `acceptance_criteria`, `issue_type`, `priority`, `labels`, `reporter`) from the response and skip to step 3.
...
4. Write `.workbench/emulator/<story-id>/story.md`:
...
3. **Persist the spec via MCP (preferred):** Call the `update_spec` workbench MCP tool with all spec fields in one call:
   ...
   **Fallback — file-only mode** (only if `update_spec` is unavailable or returns `isError: true`): skip this step and proceed with file writes only.

4. Write `.workbench/emulator/<story-id>/spec.md` for human readability regardless of MCP availability. See Spec File Format below.
...
7. On corrections:
   - **MCP mode:** Call `update_spec` with only the changed fields plus `base_revision: <revision from last response>`. Record the new `revision` from the response.
   - **File-only mode:** Edit `spec.md` directly, increment `version` in frontmatter.
   - Update `spec.md` to reflect any corrections. Repeat from step 5.
...
3. Write one task file per task: `.workbench/emulator/<story-id>/tasks/task-<story-id>-<NNN>.md`. See Task File Format below. All task files start with `status: pending`.

4. Print the task plan:
...
4. **Implement the task:**
   ...
   - Task type guidance:
     ...
     - `research`, `spec_writing`: produce a Markdown output document in `.workbench/emulator/<story-id>/`.
...
Write `.workbench/emulator/<story-id>/summary.md`:
...
Print: `Session complete. Summary → .workbench/emulator/<story-id>/summary.md`
